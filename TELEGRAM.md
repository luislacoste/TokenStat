...
TokenStat can send you a Telegram message the moment Claude Code turns ready (green) — useful if you're away from your desk and don't want to rely on the desktop notification alone.
...
1. Click the TokenStat icon in the tray → **Settings…**
...
- The Bot Token and Chat ID are stored locally in `~/.config/tokenstat/config.json` — nothing is sent anywhere except directly to `api.telegram.org`.
